...
from keras.layers import Conv1D, Dense, Embedding, Bidirectional, Dropout, GlobalAveragePooling1D, GlobalMaxPooling1D, \
    Input, LSTM, MultiHeadAttention
...
class CustomEarlyStopping(Callback):
    def __init__(self, monitor='val_loss', patience=10, min_delta=0, mode='auto', baseline=None,
                 restore_best_weights=False, verbose=0, loss_threshold=0.01):
...
def build_and_train_lstm_model(x_train, y_train, x_val, y_val, embedding_mat_columns, name, batchsize=128):
    ...
    model.add(LSTM(units=embedding_mat_columns // 4, kernel_regularizer=regularizers.l2(0.01),
    ...
    early_stopping = CustomEarlyStopping(monitor='val_loss', patience=10, min_delta=0, mode='min', baseline=None,
                                         restore_best_weights=True, verbose=2, loss_threshold=0.01)
    result = model.fit(x_train, y_train_binary, epochs=50, batch_size=batchsize, validation_data=(x_val, y_val_binary),
...
def build_and_train_lstm_glove_model(x_train, y_train, x_val, y_val, embedding_matrix, name, batchsize=128):
    ...
    model = Sequential()
    model.add(Embedding(input_dim=max_vocab, output_dim=embedding_matrix.shape[1], input_length=max_len,
...
def build_and_train_cnn_glove_model(x_train, y_train, x_val, y_val, embedding_matrix, name, batchsize=128):
    ...
    early_stopping = CustomEarlyStopping(monitor='val_loss', min_delta=0.01, patience=5, loss_threshold=0.01,
...
def build_and_train_transformer_glove_model(x_train, y_train, x_val, y_val, embedding_matrix, name, batchsize=32):
    ...
    attention_layer = MultiHeadAttention(num_heads=4, key_dim=embedding_dim // 4, dropout=0.1)(embedding_layer,
                                                                                               embedding_layer)
...
def build_and_train_transformer_glove_model(x_train, y_train, x_val, y_val, embedding_matrix, name, batchsize=128):
    ...
    embedding_layer = Embedding(input_dim=len(tokenizer.word_index) + 1, output_dim=embedding_dim,
                                weights=[embedding_matrix], input_length=max_len, trainable=False)(input_layer)
    attention_layer = MultiHeadAttention(num_heads=8, key_dim=embedding_dim // 8, dropout=0.1)(embedding_layer,
                                                                                               embedding_layer)
    pooling_layer = GlobalAveragePooling1D()(attention_layer)
    dropout_layer = Dropout(0.5)(pooling_layer)
    output_layer = Dense(1, activation='sigmoid')(dropout_layer)
...
def build_and_train_bert_classifier(x_train_raw, y_train, x_test_raw, y_test, model_name, batchsize=32):
    ...
    early_stopping = CustomEarlyStopping(monitor='val_loss', patience=10, min_delta=0, mode='min', baseline=None,
                                         restore_best_weights=True, verbose=2, loss_threshold=0.01)
    result = model.fit(x_train_encoded['input_ids'], y_train, epochs=50, batch_size=batchsize,
                       validation_data=(x_test_encoded['input_ids'], y_test), callbacks=[early_stopping])
...
def build_and_train_bert_classifier(x_train_raw, y_train, x_test_raw, y_test, model_name, batchsize=16):
    ...
    train_dataset = tf.data.Dataset.from_tensor_slices((x_train_raw, y_train)).map(
        lambda x, y: (tokenizer(x, padding=True, truncation=True, max_length=max_len, return_tensors="tf"), y)).batch(
        batchsize)
    test_dataset = tf.data.Dataset.from_tensor_slices((x_test_raw, y_test)).map(
        lambda x, y: (tokenizer(x, padding=True, truncation=True, max_length=max_len, return_tensors="tf"), y)).batch(
        batchsize)
...
for train_index, test_index in kf.split(X):
    ...
    with strategy.scope():
        model = build_and_train_electra_classifier(x_train_raw, y_train_binary, x_test_raw, y_test_binary, model_name,
                                                   batchsize)
...
with strategy.scope():
    model = build_and_train_distilbert_classifier(x_train_raw, y_train_binary, x_test_raw, y_test_binary, model_name,
                                                  batchsize)
...
with strategy.scope():
    model = build_and_train_tinybert_classifier(x_train_raw, y_train_binary, x_test_raw, y_test_binary, model_name,
                                                batchsize)
...
with strategy.scope():
    model = build_and_train_mobilebert_classifier(x_train_raw, y_train_binary, x_test_raw, y_test_binary, model_name,
                                                  batchsize)
...
def train_model(x_train, y_train, x_test, y_test, model_name, batch_size=4, epochs=1, max_len=32):
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in train_dataset:
            tokenized_inputs = tokenizer(list(inputs.numpy()), padding=True, truncation=True, max_length=max_len,
                                         return_tensors="tf")
...
y = np.argmax(labels, axis=1)
x_train_raw, x_test_raw, y_train_binary, y_test_binary = train_test_split(X, y, test_size=0.3, random_state=42)
train_model(x_train_raw, y_train_binary, x_test_raw, y_test_binary, "google/mobilebert-uncased", batch_size=4, epochs=1,
            max_len=32)
